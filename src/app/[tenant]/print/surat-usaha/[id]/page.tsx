...
import { getSuratUsahaById, type SuratUsahaData } from '@/lib/surat-usaha-actions';
...
const PrintSuratUsaha = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [suratData, settingsData] = await Promise.all([getSuratUsahaById(id), getSiteSettings(tenantId)]);
  ...
  if (!surat || !settings) return <div className="p-8 text-red-500">Data tidak ditemukan. <Link href="/dashboard/apps/surat-usaha" className="underline">Kembali</Link></div>;
    ...
    <div className="min-h-screen bg-neutral-100 print:bg-white text-black font-sans pb-12">
      <div className="print:hidden sticky top-0 z-50 bg-white border-b shadow-sm w-full py-4 px-6 flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" asChild><Link href="/dashboard/apps/surat-usaha"><ArrowLeft className="h-4 w-4 mr-2" /> Kembali</Link></Button>
          <div><h1 className="font-semibold">Pratinjau Cetak: Surat Keterangan Usaha</h1><p className="text-xs text-muted-foreground">a.n. {surat.namaPemohon}</p></div>
        </div>
      ...
      <div className="mx-auto bg-white shadow-xl print:shadow-none w-[210mm] min-h-[297mm] px-[20mm] py-[25mm] relative box-border">
        ...
        <div className="text-center mb-10">
          <h3 className="text-lg font-bold uppercase underline underline-offset-4">SURAT KETERANGAN USAHA</h3>
        ...
        <div className="ml-8 mb-6 text-[11pt]">
          <table className="w-full align-top border-0"><tbody>
            ...
            {penduduk && <><tr><td className="w-44 py-1">Pekerjaan</td><td className="w-4">:</td><td>{penduduk.pekerjaan || '-'}</td></tr>
            <tr><td className="w-44 py-1">Alamat</td><td className="w-4">:</td><td>{penduduk.alamat || '-'}</td></tr></>}
          </tbody></table>
        </div>
        <p className="text-[11pt] mb-3 text-justify">Adalah benar-benar warga kami yang menjalankan usaha dengan keterangan sebagai berikut:</p>
        <div className="ml-8 mb-6 text-[11pt]">
          <table className="w-full align-top border-0"><tbody>
            <tr><td className="w-44 py-1">Nama Usaha</td><td className="w-4">:</td><td className="font-bold">{surat.namaUsaha}</td></tr>
            <tr><td className="w-44 py-1">Jenis Usaha</td><td className="w-4">:</td><td>{surat.jenisUsaha}</td></tr>
            <tr><td className="w-44 py-1">Alamat Usaha</td><td className="w-4">:</td><td>{surat.alamatUsaha}</td></tr>
            {surat.keperluan && <tr><td className="w-44 py-1">Keperluan</td><td className="w-4">:</td><td>{surat.keperluan}</td></tr>}
          </tbody></table>
        </div>
...
export default PrintSuratUsaha;
