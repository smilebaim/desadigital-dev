...
import { getSuratKematianById, type SuratKematianData } from '@/lib/surat-kematian-actions';
...
const PrintSuratKematian = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [suratData, settingsData] = await Promise.all([getSuratKematianById(id), getSiteSettings(tenantId)]);
  ...
  if (!surat || !settings) return <div className="p-8 text-red-500">Data tidak ditemukan. <Link href="/dashboard/apps/surat-kematian" className="underline">Kembali</Link></div>;

  const today = new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
  const tglKematian = surat.tanggalKematian ? new Date(surat.tanggalKematian).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-';
    ...
    <div className="min-h-screen bg-neutral-100 print:bg-white text-black font-sans pb-12">
      <div className="print:hidden sticky top-0 z-50 bg-white border-b shadow-sm w-full py-4 px-6 flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" asChild><Link href="/dashboard/apps/surat-kematian"><ArrowLeft className="h-4 w-4 mr-2" /> Kembali</Link></Button>
          <div><h1 className="font-semibold">Pratinjau Cetak: Surat Keterangan Kematian</h1><p className="text-xs text-muted-foreground">a.n. {surat.namaPemohon}</p></div>
        </div>
      ...
      <div className="mx-auto bg-white shadow-xl print:shadow-none w-[210mm] min-h-[297mm] px-[20mm] py-[25mm] relative box-border">
        ...
        <div className="text-center mb-10">
          <h3 className="text-lg font-bold uppercase underline underline-offset-4">SURAT KETERANGAN KEMATIAN</h3>
          <p className="mt-1 text-sm">Nomor: {surat.nomorSurat || '......./......../.........'}</p>
        </div>
        <p className="text-[11pt] mb-6 text-justify">Yang bertanda tangan di bawah ini Kepala Desa {settings.siteName || '...'}, Kecamatan {settings.kecamatan || '...'}, Kabupaten {settings.kabupaten || '...'}, menerangkan bahwa telah terjadi kematian atas:</p>
        <div className="ml-8 mb-6 text-[11pt]">
          <table className="w-full align-top border-0"><tbody>
            <tr><td className="w-44 py-1">Nama Almarhum/ah</td><td className="w-4">:</td><td className="font-bold uppercase">{surat.namaPemohon}</td></tr>
            ...
            <tr><td className="w-44 py-1">Tanggal Kematian</td><td className="w-4">:</td><td className="font-semibold">{tglKematian}</td></tr>
            <tr><td className="w-44 py-1">Sebab Kematian</td><td className="w-4">:</td><td>{surat.penyebabKematian}</td></tr>
          </tbody></table>
        </div>
        <p className="text-[11pt] mb-6 text-justify">Surat keterangan ini dibuat berdasarkan keterangan dari keluarga dan saksi yang dapat dipercaya.</p>
        <p className="text-[11pt] text-justify">Demikian surat keterangan kematian ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
