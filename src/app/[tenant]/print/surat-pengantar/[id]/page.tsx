...
import { getSuratPengantarById, type SuratPengantarData } from '@/lib/surat-pengantar-actions';
...
const PrintSuratPengantar = () => {
  ...
  if (!surat || !settings) return <div className="p-8 text-red-500">Data tidak ditemukan. <Link href="/dashboard/apps/surat-pengantar" className="underline">Kembali</Link></div>;
    ...
    <div className="min-h-screen bg-neutral-100 print:bg-white text-black font-sans pb-12">
      <div className="print:hidden sticky top-0 z-50 bg-white border-b shadow-sm w-full py-4 px-6 flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" asChild><Link href="/dashboard/apps/surat-pengantar"><ArrowLeft className="h-4 w-4 mr-2" /> Kembali</Link></Button>
          <div><h1 className="font-semibold">Pratinjau Cetak: Surat Pengantar</h1><p className="text-xs text-muted-foreground">a.n. {surat.namaPemohon}</p></div>
        </div>
      ...
      <div className="mx-auto bg-white shadow-xl print:shadow-none w-[210mm] min-h-[297mm] px-[20mm] py-[25mm] relative box-border">
        ...
        <div className="text-center mb-10">
          <h3 className="text-lg font-bold uppercase underline underline-offset-4">SURAT PENGANTAR</h3>
          <p className="mt-1 text-sm">Nomor: {surat.nomorSurat || '......./......../.........'}</p>
        </div>
        <p className="text-[11pt] mb-6 text-justify">Yang bertanda tangan di bawah ini Kepala Desa {settings.siteName || '...'}, Kecamatan {settings.kecamatan || '...'}, Kabupaten {settings.kabupaten || '...'}, dengan ini menerangkan bahwa:</p>
        ...
        <p className="text-[11pt] mb-4 text-justify">Bermaksud akan mengurus keperluan: <span className="font-bold">"{surat.keperluan}"</span></p>
        <p className="text-[11pt] text-justify">Sehubungan dengan hal tersebut di atas, kami mohon kepada pihak yang berwenang untuk dapat memberikan bantuan seperlunya. Demikian surat pengantar ini dibuat untuk dapat dipergunakan sebagaimana mestinya.</p>
...
export default PrintSuratPengantar;
