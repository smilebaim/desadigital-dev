...
import { getSuratPindahById, type SuratPindahData } from '@/lib/surat-pindah-actions';
...
const PrintSuratPindah = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [suratData, settingsData] = await Promise.all([getSuratPindahById(id), getSiteSettings(tenantId)]);
  ...
  if (!surat || !settings) return <div className="p-8 text-red-500">Data tidak ditemukan. <Link href="/dashboard/apps/surat-pindah" className="underline">Kembali</Link></div>;
    ...
    <div className="min-h-screen bg-neutral-100 print:bg-white text-black font-sans pb-12">
      <div className="print:hidden sticky top-0 z-50 bg-white border-b shadow-sm w-full py-4 px-6 flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" asChild><Link href="/dashboard/apps/surat-pindah"><ArrowLeft className="h-4 w-4 mr-2" /> Kembali</Link></Button>
          <div><h1 className="font-semibold">Pratinjau Cetak: Surat Pindah</h1><p className="text-xs text-muted-foreground">a.n. {surat.namaPemohon}</p></div>
        </div>
      ...
      <div className="mx-auto bg-white shadow-xl print:shadow-none w-[210mm] min-h-[297mm] px-[20mm] py-[25mm] relative box-border">
        ...
        <div className="text-center mb-10">
          <h3 className="text-lg font-bold uppercase underline underline-offset-4">SURAT KETERANGAN PINDAH</h3>
        ...
        <div className="ml-8 mb-6 text-[11pt]">
          <table className="w-full align-top border-0"><tbody>
            ...
            <tr><td className="w-44 py-1">Alamat Asal</td><td className="w-4">:</td><td>{penduduk.alamat || '-'}</td></tr></>}
            <tr><td className="w-44 py-1">Alamat Tujuan</td><td className="w-4">:</td><td className="font-semibold">{surat.alamatTujuan}</td></tr>
            <tr><td className="w-44 py-1">Alasan Pindah</td><td className="w-4">:</td><td>{surat.alasanPindah}</td></tr>
          </tbody></table>
        </div>
        <p className="text-[11pt] text-justify">Demikian surat keterangan pindah ini dibuat dengan sebenarnya untuk dipergunakan sebagaimana mestinya.</p>
...
export default PrintSuratPindah;
