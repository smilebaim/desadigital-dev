...
import { addSuratKeterangan, updateSuratKeterangan, deleteSuratKeterangan, type SuratKeteranganData } from "@/lib/surat-keterangan-actions";
...
interface Surat extends SuratKeteranganData {
  id: string;
  createdAt: any;
}
...
const suratKeteranganSchema = z.object({
  pendudukId: z.string().min(1, "Pemohon wajib dipilih"),
  keterangan: z.string().min(1, "Isi keterangan wajib diisi"),
});

type FormValues = z.infer<typeof suratKeteranganSchema>;
...
const SuratKeteranganPage = () => {
    ...
    const { control, handleSubmit, reset, formState: { isSubmitting, errors } } = useForm<FormValues>({
        resolver: zodResolver(suratKeteranganSchema),
        defaultValues: { pendudukId: "", keterangan: "" }
    });

    useEffect(() => {
        if (isTenantLoading) return;
        const unsubSurat = getSuratKeteranganStream((data) => {
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const data: Omit<SuratKeteranganData, 'createdAt'> = {
            ...values,
        ...
        let result;
        if (formMode === 'add') {
            result = await addSuratKeterangan(data);
        } else if (selectedSurat) {
            result = await updateSuratKeterangan(selectedSurat.id, data);
        }
    ...
    const handleStatusChange = async (suratId: string, status: 'Selesai' | 'Ditolak') => {
        const result = await updateSuratKeterangan(suratId, { status });
        if (result.success) {
    ...
    const handleDelete = async () => {
        ...
        const result = await deleteSuratKeterangan(selectedSurat.id);
    ...
    return (
        <>
                                ...
                                {loading ? (
                                    ...
                                    filteredSurat.map((surat) => (
                                        <TableRow key={surat.id}>
                                            ...
                                            <TableCell className="max-w-[200px] truncate">{surat.keterangan}</TableCell>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        {surat.status === 'Selesai' && (
                                                            <DropdownMenuItem onClick={() => window.open(`/print/surat-keterangan/${surat.id}`, '_blank')}>
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{formMode === 'add' ? 'Buat Surat Keterangan Baru' : 'Edit Permohonan'}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="keterangan">Isi Keterangan</Label>
                                <Controller name="keterangan" control={control} render={({ field }) => <Textarea id="keterangan" {...field} placeholder="Jelaskan isi atau perihal surat keterangan..." />} />
                                {errors.keterangan && <p className="text-xs text-red-500">{errors.keterangan.message}</p>}
                            </div>
                        </div>
                        <DialogFooter>
