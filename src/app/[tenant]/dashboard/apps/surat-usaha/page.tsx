...
import { addSuratUsaha, updateSuratUsaha, deleteSuratUsaha, type SuratUsahaData } from "@/lib/surat-usaha-actions";
...
const suratUsahaSchema = z.object({
...
type FormValues = z.infer<typeof suratUsahaSchema>;

const SuratUsahaPage = () => {
    ...
    const { control, handleSubmit, reset, setValue, formState: { isSubmitting, errors } } = useForm<FormValues>({
        resolver: zodResolver(suratUsahaSchema),
        defaultValues: { pendudukId: "", namaUsaha: "", jenisUsaha: "", alamatUsaha: "" }
    });
    ...
    const openAddForm = () => {
        ...
        reset({ pendudukId: "", namaUsaha: "", jenisUsaha: "", alamatUsaha: "" });
        setIsFormOpen(true);
    };
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const data: Omit<SuratUsahaData, 'createdAt'> = {
            ...values,
        ...
        let result;
        if (formMode === 'add') {
            result = await addSuratUsaha(data);
        } else if (selectedSurat) {
            result = await updateSuratUsaha(selectedSurat.id, data);
        }
    ...
    const handleStatusChange = async (suratId: string, status: 'Selesai' | 'Ditolak') => {
        const result = await updateSuratUsaha(suratId, { status });
        if (result.success) {
    ...
    const handleDelete = async () => {
        ...
        const result = await deleteSuratUsaha(selectedSurat.id);
    ...
    const filteredSurat = suratList.filter(s => 
        ...
        s.namaUsaha.toLowerCase().includes(searchQuery.toLowerCase())
    );
        ...
        <>
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight">Aplikasi Surat Usaha</h2>
                        <p className="text-muted-foreground">Kelola surat keterangan usaha dari sini.</p>
                    </div>
                    ...
                    <CardHeader>
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <CardDescription>Total surat: {suratList.length}</CardDescription>
                            </div>
                            <div className="relative">
                                <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                                <Input placeholder="Cari pemohon, NIK, atau nama usaha..." className="pl-8" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent>
                                ...
                                {loading ? (
                                    ...
                                    filteredSurat.map((surat) => (
                                        <TableRow key={surat.id}>
                                            ...
                                            <TableCell>{surat.namaUsaha}</TableCell>
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent className="sm:max-w-[625px]">
                    <DialogHeader>
                        <DialogTitle>{formMode === 'add' ? 'Buat Surat Keterangan Usaha' : 'Edit Surat'}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Controller name="namaUsaha" control={control} render={({ field }) => <Input id="namaUsaha" {...field} />} />
                                {errors.namaUsaha && <p className="text-xs text-red-500">{errors.namaUsaha.message}</p>}
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="jenisUsaha">Jenis Usaha</Label>
                                <Controller name="jenisUsaha" control={control} render={({ field }) => <Input id="jenisUsaha" {...field} />} />
                                {errors.jenisUsaha && <p className="text-xs text-red-500">{errors.jenisUsaha.message}</p>}
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="alamatUsaha">Alamat Usaha</Label>
                                <Controller name="alamatUsaha" control={control} render={({ field }) => <Input id="alamatUsaha" {...field} />} />
                                {errors.alamatUsaha && <p className="text-xs text-red-500">{errors.alamatUsaha.message}</p>}
                            </div>
                        </div>
                        <DialogFooter>
