...
import { addSuratPengantar, updateSuratPengantar, deleteSuratPengantar, type SuratPengantarData } from "@/lib/surat-pengantar-actions";
...
const suratPengantarSchema = z.object({
...
const SuratPengantarPage = () => {
    ...
    const { control, handleSubmit, reset, formState: { isSubmitting, errors } } = useForm<FormValues>({
        resolver: zodResolver(suratPengantarSchema),
        defaultValues: { pendudukId: "", keperluan: "" }
    });
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const data: Omit<SuratPengantarData, 'createdAt'> = {
            ...values,
        ...
        let result;
        if (formMode === 'add') {
            result = await addSuratPengantar(data);
        } else if (selectedSurat) {
            result = await updateSuratPengantar(selectedSurat.id, data);
        }
    ...
    const handleStatusChange = async (suratId: string, status: 'Selesai' | 'Ditolak') => {
        const result = await updateSuratPengantar(suratId, { status });
        if (result.success) {
    ...
    const handleDelete = async () => {
        ...
        const result = await deleteSuratPengantar(selectedSurat.id);
    ...
    return (
        <>
                                ...
                                {loading ? (
                                    ...
                                    filteredSurat.map((surat) => (
                                        <TableRow key={surat.id}>
                                            ...
                                            <TableCell className="max-w-[200px] truncate">{surat.keperluan}</TableCell>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        {surat.status === 'Selesai' && (
                                                            <DropdownMenuItem onClick={() => window.open(`/print/surat-pengantar/${surat.id}`, '_blank')}>
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{formMode === 'add' ? 'Buat Surat Pengantar Baru' : 'Edit Permohonan'}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Controller name="keperluan" control={control} render={({ field }) => <Textarea id="keperluan" {...field} placeholder="Contoh: Pengantar pembuatan SKCK" />} />
