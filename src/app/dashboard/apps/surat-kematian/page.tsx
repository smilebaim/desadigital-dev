...
import { addSuratKematian, updateSuratKematian, deleteSuratKematian, type SuratKematianData } from "@/lib/surat-kematian-actions";
...
const suratKematianSchema = z.object({
  pendudukId: z.string().min(1, "Penduduk wajib dipilih"),
  tanggalKematian: z.string().min(1, "Tanggal kematian wajib diisi"),
  penyebabKematian: z.string().min(1, "Penyebab kematian wajib diisi"),
});

type FormValues = z.infer<typeof suratKematianSchema>;
...
const SuratKematianPage = () => {
    ...
    const { control, handleSubmit, reset, formState: { isSubmitting, errors } } = useForm<FormValues>({
        ...
        defaultValues: { pendudukId: "", tanggalKematian: "", penyebabKematian: "" }
    });

    useEffect(() => {
        const unsubSurat = getSuratKematianStream((data) => {
    ...
    const openAddForm = () => {
        ...
        reset({ pendudukId: "", tanggalKematian: "", penyebabKematian: "" });
        setIsFormOpen(true);
    };
    ...
    const openEditForm = (surat: Surat) => {
        ...
        reset({
            ...
            penyebabKematian: surat.penyebabKematian,
        });
        setIsFormOpen(true);
    };
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        const data: Omit<SuratKematianData, 'createdAt'> = {
            ...values,
        ...
        let result;
        if (formMode === 'add') {
            result = await addSuratKematian(data);
        } else if (selectedSurat) {
            result = await updateSuratKematian(selectedSurat.id, data);
        }
    ...
    const handleStatusChange = async (suratId: string, status: 'Selesai' | 'Ditolak') => {
        const result = await updateSuratKematian(suratId, { status });
        if (result.success) {
    ...
    const handleDelete = async () => {
        ...
        const result = await deleteSuratKematian(selectedSurat.id);
    ...
    return (
        <>
                    ...
                    <CardContent>
                        <div className="flex justify-between items-center mb-4">
                            <div className="relative">
                                ...
                                <Input placeholder="Cari nama almarhum/ah..." className="pl-8" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                            </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <TableHead>Nama Almarhum/ah</TableHead>
                                ...
                                {loading ? (
                                    ...
                                    filteredSurat.map((surat) => (
                                        <TableRow key={surat.id}>
                                            ...
                                            <TableCell>{new Date(surat.tanggalKematian).toLocaleDateString('id-ID')}</TableCell>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        {surat.status === 'Selesai' && (
                                                            <DropdownMenuItem onClick={() => window.open(`/print/surat-kematian/${surat.id}`, '_blank')}>
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{formMode === 'add' ? 'Buat Surat Keterangan Kematian' : 'Edit Permohonan'}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="grid gap-4 py-4">
                            <div className="space-y-2">
                                <Label htmlFor="pendudukId">Penduduk yang Meninggal</Label>
                            ...
                            <div className="space-y-2">
                                ...
                                <Controller name="tanggalKematian" control={control} render={({ field }) => <Input id="tanggalKematian" type="date" {...field} />} />
                                {errors.tanggalKematian && <p className="text-xs text-red-500">{errors.tanggalKematian.message}</p>}
                            </div>
                             <div className="space-y-2">
                                <Label htmlFor="penyebabKematian">Penyebab Kematian</Label>
                                <Controller name="penyebabKematian" control={control} render={({ field }) => <Textarea id="penyebabKematian" {...field} />} />
                                {errors.penyebabKematian && <p className="text-xs text-red-500">{errors.penyebabKematian.message}</p>}
                            </div>
                        </div>
                        <DialogFooter>
