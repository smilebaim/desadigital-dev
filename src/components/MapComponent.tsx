...
const BIDANG_TANAH_BOUNDARY: [number, number][] = [
  [-1.21, 104.38],
  [-1.21, 104.385],
  [-1.215, 104.385],
  [-1.215, 104.38],
...
export const MapComponent = () => {
  ...
  const bidangTanahBoundaryRef = useRef<Polygon | null>(null);
  ...
  useEffect(() => {
    ...
    if (activeLayers.includes('Bidang Tanah')) {
      if (!bidangTanahBoundaryRef.current) {
        bidangTanahBoundaryRef.current = L.polygon(BIDANG_TANAH_BOUNDARY, { color: 'yellow' })
        .bindPopup('<b>Bidang Tanah</b>')
        .on('click', () => {
          setMarkerInfo({
            title: 'Informasi Bidang Tanah',
            description: 'Ini adalah contoh informasi untuk sebidang tanah.',
            type: 'bidang'
          });
          setInfoPanelExpanded(true);
        });
      }
      bidangTanahBoundaryRef.current.addTo(map);
    } else {
      bidangTanahBoundaryRef.current?.remove();
    }
  }, [activeLayers]);
