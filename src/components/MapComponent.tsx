...
export const MapComponent: React.FC = () => {
  ...
  const [markerInfo, setMarkerInfo] = useState<{ title: string; coordinates?: LatLngTuple; description: string; type?: 'marker' | 'boundary' } | null>(null);
  ...
  // Handle administrative boundary layer
  useEffect(() => {
    if (!mapRef.current) return;

    if (activeLayers.includes('Peta Administrasi')) {
      if (!administrativeBoundaryRef.current) {
        administrativeBoundaryRef.current = L.polygon(ADMINISTRATIVE_BOUNDARY, {
          color: 'blue',
          weight: 2,
          opacity: 0.7,
          fillOpacity: 0.2
          ...
          setMarkerInfo({
            title: 'Batas Administrasi Desa',
            description: 'Ini adalah batas wilayah administrasi Desa Remau Bako Tuo.',
            type: 'boundary'
          });
          setInfoPanelOpen(true);
        });
      }
      administrativeBoundaryRef.current.addTo(mapRef.current);
    } else {
      if (administrativeBoundaryRef.current) {
